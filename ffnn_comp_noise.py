...
if __name__ == "__main__":
    ...
    # noise
    noise_on_train_flag = False
    noise_on_test_flag = False
    ...
    ax.set_ylim(0, 1.0)
    xplot = [[] for i in range(nl)]
    yplot = [[] for i in range(nl)]
    ...
    #"""
    X_rand = [X_test.copy() for i in noise_list]
    for i, noise in enumerate(noise_list):
        for n in range(10000):
            num_rand = np.random.binomial(n=784, p=noise)
